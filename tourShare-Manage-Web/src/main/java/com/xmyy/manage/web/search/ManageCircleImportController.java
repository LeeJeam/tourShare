 ...
 */
@RestController
@RequestMapping("/manage/cricle/import")
@Api(value = "动态ES索引库导入", description = "动态ES索引库导入")
public class ManageCircleImportController extends AbstractController {
    ...
    public Callable<Object> fullImportDatas() {
        return () -> {
            ...
            t.start();
            int importCount = circleImportService.fullImportCirclesToIndex(-1);
            t.stop();
    ...
    public Callable<Object> incrementImportDatas() {
        return () -> {
            int importCount = circleImportService.incrementImportCirclesToIndex(-1);
    ...
    public Object rebuild() {
        String msg = "";
        if (circleImportService.rebuildIndex()) {
            int importCount = circleImportService.fullImportCirclesToIndex(3000);
            msg += "导入动态圈库索引成功！共导入[" + importCount + "]条动态圈数据！";
        } else {
            msg += "导入动态圈库索引失败！";
        }
