 ...
 */
@RestController
@RequestMapping("/manage/seller/import")
@Api(value = "买手ES索引库导入", description = "买手ES索引库导入")
public class ManageSellerImportController extends AbstractController {
    ...
    public Callable<Object> fullImportDatas() {
        return () -> {
            ...
            t.start();
            int importCount = sellerService.fullImportSellersToIndex(-1);
            t.stop();
    ...
    public Object rebuild() {
        String msg = "";
        if (sellerService.rebuildIndex()) {
            int importCount = sellerService.fullImportSellersToIndex(3000);
            msg += "导入买手库索引成功！共导入[" + importCount + "]条买手数据！";
        } else {
            msg += "导入买手库索引失败！";
        }
